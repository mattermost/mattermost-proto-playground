...
| **7 — Subpath imports** | Done | Playground on `@mattermost/compass-ui@0.1.0-alpha.6+` with `@mattermost/compass-ui/components/<kebab>` imports (not root barrel). |
...
- **Playground (this repo):** `@mattermost/compass-ui@alpha` from npm (`0.1.0-alpha.6+`, subpath imports) + `file:../compass-design/packages/compass-proto`
