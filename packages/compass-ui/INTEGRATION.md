...
Form widgets with menu/popover surfaces (`Combobox`, `Select` on `Menu`, `DateRangePicker`, etc.) are exempt — they manage widget-level dropdown/picker open state. Proto/mobile presentation shells with backdrop or sheet animation live in `@mattermost/compass-proto`, not in published `compass-ui`.
