...
export function agentAvatarChipSrc(
  ...
  const key = `${shape}:${color}:eyes:${fullSize ? 'full' : 'chip'}`;
  ...
  // In chip contexts, scale non-sphere shapes to 75% so the farthest corners
  // fit within the inscribed circle when UserAvatar applies border-radius: full.
  // In fullSize contexts (e.g. profile popover) the circle is removed, so we
  // let the shape fill the whole viewBox for consistent visual weight.
  const s = fullSize ? 1 : 0.75;
  const scale = (shape === 'sphere' || fullSize) ? '' : ` transform="translate(0.5 0.5) scale(${s}) translate(-0.5 -0.5)"`;
  const body =
    shape === 'sphere'
