...
export interface ValueEditorPopoverProps {
  ...
  onShowUnder: (parentId: string) => void;
 ...
 *  - There is no "primary parent". The parents list is ordered by the parent's
 *    own creation time everywhere, the home occurrence is tagged "Shown here",
 *    and the action to move it is "Show under this parent" — display words, for
 *    a display-only setting, on a list whose helper text says outright that all
      ...
      {pane === 'main' && (
        <div className={styles['editor']}>
          ...
          >
            ...
            <span className={styles['editor__nav-label']}>Children</span>
            <span className={styles['editor__nav-value']}>
            ...
            {parents.map((parent) => (
              <div key={parent.id} className={styles['editor__row']}>
                <div className={styles['editor__row-top']}>
                  ...
                  {parent.id === homeId ? (
                    <LabelTag label="Shown here" type="Info Dim" size="Small" />
                  ) : (
                    <button
                      type="button"
                      className={styles['editor__show-here']}
                      onClick={() => onShowUnder(parent.id)}
                    >
                      Show under this parent
                    </button>
                  )}
