...
export function isReferenceOccurrence(
  ...
  if (viaParentId == null) return false; // a root is always its own home
  return homeParentIdOf(values, value) !== viaParentId;
}
...
/** The enumerated form, for the chip's tooltip only. */
export function alsoUnderText(
  values: HierValue[],
  value: HierValue,
): string | null {
  const others = otherParentsOf(values, value);
  if (others.length === 0) return null;
  return `Also under ${formatList(others.map((p) => p.label))}`;
}
