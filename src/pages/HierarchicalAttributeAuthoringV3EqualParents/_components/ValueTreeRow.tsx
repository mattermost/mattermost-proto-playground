 ...
 *     rendered, always. A value with several parents shows in full under each
 *     parent, with a link-icon parent count when it has more than one.
 *
 ...
 *
 *  3. EVERY ROW IS A FIRST-CLASS POSITION. A value with several parents renders
 *     identically under each one — full edit controls, children, and per-parent
 *     reordering. Dragging a row touches only the edge THAT ROW REPRESENTS.
 *
...
export default function ValueTreeRow({
  ...
  const children = siblingsOf(values, value.id);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {parentCount && (
          <HoverTip label={parentNames ?? `${value.parentIds.length} parents`}>
            <button
              type="button"
      ...
      {expanded && children.length > 0 && (
        <div
          role="group"
          aria-label={`Values granted by ${value.label}`}
        >
          {children.map((child) => (
