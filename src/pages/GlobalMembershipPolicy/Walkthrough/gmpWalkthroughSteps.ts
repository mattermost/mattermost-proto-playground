...
export const GMP_WALKTHROUGH_STEPS: WalkthroughStep[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      'The value field is filtered the same way — only values and channel attributes matching that attribute type',
  ...
  },
  {
    ...
    bullets: [
      ...
      'The value field groups two kinds of option: fixed Values, and Channel attributes that vary per channel',
      'Only type-compatible options appear: classification-level values and the Channel:Classification attribute, not unrelated ones',
      'Channel attributes render as distinct tokens so you can tell a per-channel variable from a fixed value',
    ...
    lookFor: [
      ...
      'Value field groups: Values (fixed) and Channel attributes (per-channel variables)',
    ],
    preview: { kind: 'iframe', path: withFocus(DS_EDITOR, 'value-picker') },
  },
  {
    id: 'literal-example',
    section: 'list',
