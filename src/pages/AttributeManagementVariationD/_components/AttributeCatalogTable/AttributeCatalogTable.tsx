...
export type ValuesDisplayMode = 'chips' | 'count';

export interface AttributeCatalogTableProps {
  attributes: AttributeD[];
  /** Full catalog for sibling name resolution. */
  allAttributes?: AttributeD[];
  resourceFilter: ResourceFilterKey;
  /** Catalog table shows value chips (default) or a count only. */
  valuesDisplay?: ValuesDisplayMode;
...
function visibleValues(attr: AttributeD) {
...
function ValueChips({
  ...
  attr: AttributeD;
  values: Array<{ id: string; label: string; rank?: number }>;
}) {
  return (
...
function SyncProvenance({
  ...
  sync: ReturnType<typeof relativeSync>;
}) {
  return (
    <span
...
function ValueCount({
  ...
  return (
    <div className={styles['table__values']}>
      <span className={styles['table__values-count']}>
        {formatValueCount(count)}
      </span>
      {disabledCount != null && disabledCount > 0 && resourceFilter && resourceFilter !== 'All' && (
        <p className={styles['table__overlay-note']}>
          {disabledCount} disabled for new {resourceFilter.toLowerCase()}{' '}
          assignments
        </p>
      )}
    </div>
...
function ValuesCell({
  ...
  if (resourceFilter === 'All') {
    if (sync) {
      return <SyncProvenance sync={sync} />;
    }
    if (valuesDisplay === 'count') {
      return (
        <ValueCount count={visibleValues(attr).length} />
      );
    }
    return <ValueChips attr={attr} values={visibleValues(attr)} />;
  }

  const binding = getBinding(attr, resourceFilter);
  if (!binding) {
    return <span className={styles['table__values-empty']}>—</span>;
  }

  const effective = effectiveValuesForBinding(attr, binding);
  const enabled = effective.filter((v) => !v.disabledForNew);
  const disabledCount = effective.filter((v) => v.disabledForNew).length;
  ...
  return (
    <div className={styles['table__values']}>
      ...
      {effective.some((v) => v.disabledForNew) && (
        <p className={styles['table__overlay-note']}>
          {effective.filter((v) => v.disabledForNew).length} disabled for new{' '}
          {resourceFilter.toLowerCase()} assignments
        </p>
      )}
    </div>
  );
