...
import styles from './AttributeManagementVariationD.module.scss';

/**
 * Attribute Management — Variation D (table catalog + per-resource values)
 *
 * Extends Variation C with per-resource value overlays: admins can disable
 * base catalog values for new assignments on a specific resource (without
 * deleting them). Global catalog remains canonical.
 */
export default function AttributeManagementVariationD() {
  const initialParams =
        ...
        {selected ? (
          ...
          <>
            <PageHeader
              ...
              description="Define an attribute once, then customize allowed values per resource without deleting from the global catalog."
