...
import { resolveValueLink } from '@/pages/AttributeHubSimplified/_components/simplifiedModel';
...
export interface ChannelAttributesWorkspaceProps {
  onDirtyChange?: (dirty: boolean) => void;
}

export default function ChannelAttributesWorkspace({
  onDirtyChange,
  ...
  const addResource = (resource: ResourceKind) => {
    if (!CHANNEL_SCOPE_RESOURCES.includes(resource)) return;
    mutate((a) =>
      a.appliesTo.some((c) => c.resource === resource)
        ? a
        : { ...a, appliesTo: [...a.appliesTo, defaultChannelResourceConfig(resource)] },
    );
  };
  ...
  const startCreate = () => {
    setSelectedId(null);
    setDraft(blankChannelAttribute());
    markDirty();
  };
      ...
      {active ? (
        <>
          ...
          <SimplifiedDetailView
            ...
            onSetValueRank={(valueId, tier) =>
              mutate((a) => ({
                ...a,
                values: a.values.map((v) =>
                  v.id === valueId ? { ...v, tier } : v,
                ),
              }))
            }
        ...
        <>
          <div className={styles['workspace__list-header']}>
            <h3 className={styles['workspace__list-title']}>{CHANNEL_CATALOG_TITLE}</h3>
            <p className={styles['workspace__list-sub']}>{CHANNEL_ATTRIBUTES_INTRO}</p>
          </div>
          <CatalogListing
            attributes={attributes}
            filtered={filtered}
