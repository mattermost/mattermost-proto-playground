...
const QUICK_TYPES: AttrType[] = ATTR_TYPE_OPTIONS;
...
export default function QuickCreateAttributeModal({
  ...
  const canCreate =
    trimmed.length > 0 &&
    (!takesValueListForType(type) || values.length > 0);
  ...
  const handleCreate = () => {
    ...
    const attribute: HubAttribute = {
      ...
      type,
      description: '',
      values: takesValueListForType(type) ? values : [],
      source: { kind: 'manual' },
              ...
              >
                {QUICK_TYPES.map((option) => (
                  <option key={option} value={option}>
                    {attrTypeLabel(option)}
                  </option>
                ))}
              </Select>
            </label>

            {takesValueListForType(type) && (
