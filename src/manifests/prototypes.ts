...
export const PROTOTYPES: PrototypeEntry[] = [
  ...
  // Team + Channel settings modals — simplified hub scoped per resource level (2026-07-07)
  {
    ...
    group: 'zero-trust-abac',
    description:
      'Simplified attribute hub inside Team settings and Channel settings modals, plus channel thread and info views. Top tabs switch between Team settings, Channel settings, Channel · thread, Channel info Admin, and Channel · info user (read-only channel attributes for regular members). Team settings: one catalog for attributes applying to the team, its channels, and posts. Channel settings: catalog for this channel and posts. Channel · thread: open thread RHS lists all post attributes on the root message (inherited, overridden, locked); + Add attribute opens Channel settings on the new-attribute flow with Posts pre-selected in Applies to. Channel info Admin: editable channel attributes in the info sidebar (channel admins can change Program value); sidebar header + opens a menu with Create new channel (and Browse channels / Open a direct message / etc.), which launches the Create a new channel modal with public/private type cards, name, purpose, and channel-scoped attributes (Classification locked to SECRET, Programs multiselect, Mission tag select, + Add attribute). Channel · info user: same layout with read-only attribute labels and values — no add, edit, or remove. Deep-links: ?view=team|channel|channel-thread|channel-info|channel-info-user (?view=create-channel falls back to channel-info), ?tab=attributes, ?attr=<id>, ?flow=new, ?applies=Posts|Channels|Posts,Channels.',
    addedAt: '2026-07-07',
    isPrimary: true,
    collections: ['attribute-management'],
